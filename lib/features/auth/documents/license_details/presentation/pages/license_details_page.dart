...
class _LicenseDetailsPageState extends State<LicenseDetailsPage> {
  ...
  Future<void> _loadDraft() async {
    final draft = await DocumentDraftService.getLicenseDraft();
    if (!mounted) return;
    setState(() {
    ...
    final frontPath = draft['frontPath'] ?? '';
    if (frontPath.isNotEmpty) {
      final file = File(frontPath);
      if (await file.exists() && mounted) {
        setState(() => _frontPhoto = file);
      }
    }

    final backPath = draft['backPath'] ?? '';
    if (backPath.isNotEmpty) {
      final file = File(backPath);
      if (await file.exists() && mounted) {
        setState(() => _backPhoto = file);
      }
    }
  }

  Future<void> _saveDraft() async {
    await DocumentDraftService.saveLicenseDraft(
  ...
  Future<void> _save() async {
    ...
    await DocumentDraftService.clearLicenseDraft();
