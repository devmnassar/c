...
List<LatLng> buildSmoothCurvePoints(
  ...
  for (var i = 0; i <= numPoints; i++) {
    ...
    final tt = t * t;
    points.add(LatLng(
      mt2 * start.latitude + 2 * mt * t * control.latitude + tt * end.latitude,
      mt2 * start.longitude +
...
class _IncomingOrderPageState extends State<IncomingOrderPage> {
  ...
  void _runInitialFitOnce(List<LatLng> points, double mapHeight) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await fitToTwoPoints(_mapController!, points[0], points[1],
          mapHeight: mapHeight);
              ...
              return Stack(
                ...
                children: [
                  ...
                  IncomingMapControls(
                    ...
                    onExpand: () => context.push('/incoming-order/map',
                        extra: widget.order),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.fromLTRB(
          compact ? 8 : 12, compact ? 5 : 8, compact ? 8 : 12, 0),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          // Row A: title | chip (no overflow)
          Row(
          ...
          // Row B: timer (full width, responsive)
          if (countdown != null) ...[
