...
class _IqamaDetailsPageState extends State<IqamaDetailsPage> {
  ...
  void initState() {
    super.initState();
    _loadDraft();
    _givenNameController.addListener(_saveDraft);
    _surnameController.addListener(_saveDraft);
    _dobController.addListener(_saveDraft);
    _numberController.addListener(_saveDraft);
    _expiryController.addListener(_saveDraft);
  }

  Future<void> _loadDraft() async {
    final draft = await DocumentDraftService.getIqamaDraft();
    if (!mounted) return;
    setState(() {
      _givenNameController.text = draft['givenName'] ?? '';
      _surnameController.text = draft['surname'] ?? '';
      _dobController.text = draft['dob'] ?? '';
      _gender = (draft['gender'] ?? '').isEmpty ? null : draft['gender'];
      _numberController.text = draft['number'] ?? '';
      _expiryController.text = draft['expiryDate'] ?? '';
    });
  ...
  Future<void> _saveDraft() async {
    await DocumentDraftService.saveIqamaDraft(
  ...
  Future<void> _save() async {
    ...
    await DocumentDraftService.clearIqamaDraft();
  ...
  @override
  void dispose() {
    _givenNameController.removeListener(_saveDraft);
    _surnameController.removeListener(_saveDraft);
    _dobController.removeListener(_saveDraft);
    _numberController.removeListener(_saveDraft);
    _expiryController.removeListener(_saveDraft);
